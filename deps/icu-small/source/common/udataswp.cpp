...
udata_swapDataHeader(const UDataSwapper *ds,
    ...
    if( (length>=0 && length<(int32_t)sizeof(DataHeader)) ||
        ...
        udata_printError(ds, u8"udata_swapDataHeader(): initial bytes do not look like ICU data\n");
    ...
    if( headerSize<sizeof(DataHeader) ||
        ...
        udata_printError(ds, u8"udata_swapDataHeader(): header size mismatch - headerSize %d infoSize %d length %d\n",
