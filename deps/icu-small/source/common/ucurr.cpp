...
static const char CURRENCY_DATA[] = u8"supplementalData";
// Tag for meta-data, in root.
static const char CURRENCY_META[] = u8"CurrencyMeta";

// Tag for map from countries to currencies, in root.
static const char CURRENCY_MAP[] = u8"CurrencyMap";

// Tag for default meta-data, in CURRENCY_META
static const char DEFAULT_META[] = u8"DEFAULT";

// Variant for legacy pre-euro mapping in CurrencyMap
static const char VAR_PRE_EURO[] = u8"PREEURO";

// Variant for legacy euro mapping in CurrencyMap
static const char VAR_EURO[] = u8"EURO";

// Variant delimiter
static const char VAR_DELIM = '\x5f';
static const char VAR_DELIM_STR[] = u8"_";
...
static const char CURRENCIES[] = u8"Currencies";
static const char CURRENCYPLURALS[] = u8"CurrencyPlurals";
...
static int32_t countEquivalent(const icu::Hashtable &hash, const icu::UnicodeString &s) {
 ...
 {
   char tmp[200];
   s.extract(0,s.length(),tmp, u8"UTF-8");
   printf(u8"CountEquivalent('%s') = %d\n", tmp, result);
 }
#endif
    return result;
}
                ...
                UErrorCode* ec)
{
    ...
    if (ec != NULL && U_SUCCESS(*ec)) {
        if ((buff && buffCapacity) || !buffCapacity) {
            ...
            char id[ULOC_FULLNAME_CAPACITY];
            if ((resLen = uloc_getKeywordValue(locale, u8"currency", id, ULOC_FULLNAME_CAPACITY, &localStatus))) {
                ...
                s = ures_getStringByKey(currencyReq, u8"id", &resLen, &localStatus);
                ...
                */
                ...
                if ((U_FAILURE(localStatus)) && strchr(id, '\x5f') != 0)
                {
...
ucurr_getPluralName(const UChar* currency,
    ...
    if (U_FAILURE(ec2)) {
        //  fall back to "other"
        ec2 = U_ZERO_ERROR;
        s = ures_getStringByKeyWithFallback(rb, u8"other", len, &ec2);
        if (U_FAILURE(ec2)) {
            ures_close(rb);
...
collectCurrencyNames(const char* locale,
    ...
    *total_currency_symbol_count = 0;
...
#ifdef UCURR_DEBUG
    printf(u8"currency name count: %d\n", *total_currency_name_count);
    for (int32_t index = 0; index < *total_currency_name_count; ++index) {
        ...
        printf(u8"currencyName: %s\n", curNameBuf);
        printf(u8"len: %d\n", (*currencyNames)[index].currencyNameLen);
    }
    printf(u8"currency symbol count: %d\n", *total_currency_symbol_count);
...
linearSearch(const CurrencyNameStruct* currencyNames,
    ...
    for (int32_t index = begin; index <= end; ++index) {
        ...
        if (len > *maxMatchLen && len <= textLen &&
            ...
            *maxMatchLen = len;
#ifdef UCURR_DEBUG
            printf(u8"maxMatchIndex = %d, maxMatchLen = %d\n",
...
#ifdef UCURR_DEBUG
    printf(u8"search in names, max = %d, matchIndex = %d\n", max, matchIndex);
#endif
...
#ifdef UCURR_DEBUG
    printf(u8"search in symbols, maxInSymbol = %d, matchIndexInSymbol = %d\n", maxInSymbol, matchIndexInSymbol);
...
static const struct CurrencyList {
    ...
    {u8"ADP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"AED", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"AFA", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"AFN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ALK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ALL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"AMD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ANG", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"AOA", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"AOK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"AON", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"AOR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ARA", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ARL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ARM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ARP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ARS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ATS", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"AUD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"AWG", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"AZM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"AZN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BAD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BAM", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BAN", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BBD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BDT", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BEC", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"BEF", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BEL", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"BGL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BGM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BGN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BGO", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BHD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BIF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BMD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BND", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BOB", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BOL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BOP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BOV", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"BRB", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BRC", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BRE", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BRL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BRN", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BRR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BRZ", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BSD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BTN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BUK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BWP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BYB", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"BYR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"BZD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CAD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CDF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CHE", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"CHF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CHW", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"CLE", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"CLF", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"CLP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CNX", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"CNY", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"COP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"COU", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"CRC", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CSD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"CSK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"CUC", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CUP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CVE", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"CYP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"CZK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"DDM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"DEM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"DJF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"DKK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"DOP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"DZD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ECS", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ECV", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"EEK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"EGP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"EQE", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ERN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ESA", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"ESB", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"ESP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ETB", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"EUR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"FIM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"FJD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"FKP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"FRF", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"GBP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GEK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"GEL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GHC", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"GHS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GIP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GMD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GNF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GNS", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"GQE", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"GRD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"GTQ", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GWE", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"GWP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"GYD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"HKD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"HNL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"HRD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"HRK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"HTG", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"HUF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"IDR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"IEP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ILP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ILR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ILS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"INR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"IQD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"IRR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ISJ", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ISK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ITL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"JMD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"JOD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"JPY", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KES", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KGS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KHR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KMF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KPW", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KRH", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"KRO", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"KRW", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KWD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KYD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"KZT", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LAK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LBP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LKR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LRD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LSL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LSM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"LTL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LTT", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"LUC", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"LUF", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"LUL", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"LVL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"LVR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"LYD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MAD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MAF", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MCF", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MDC", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MDL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MGA", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MGF", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MKD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MKN", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MLF", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MMK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MNT", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MOP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MRO", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MTL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MTP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MUR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MVP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MVR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MWK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MXN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MXP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MXV", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"MYR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MZE", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"MZM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"MZN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"NAD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"NGN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"NIC", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"NIO", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"NLG", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"NOK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"NPR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"NZD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"OMR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"PAB", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"PEI", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"PEN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"PES", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"PGK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"PHP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"PKR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"PLN", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"PLZ", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"PTE", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"PYG", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"QAR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"RHD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ROL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"RON", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"RSD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"RUB", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"RUR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"RWF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SAR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SBD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SCR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SDD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"SDG", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SDP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"SEK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SGD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SHP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SIT", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"SKK", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SLL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SOS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SRD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SRG", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"SSP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"STD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SUR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"SVC", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SYP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"SZL", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"THB", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TJR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"TJS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TMM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"TMT", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TND", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TOP", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TPE", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"TRL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"TRY", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TTD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TWD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"TZS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"UAH", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"UAK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"UGS", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"UGX", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"USD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"USN", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"USS", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"UYI", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"UYP", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"UYU", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"UZS", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"VEB", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"VEF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"VND", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"VNN", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"VUV", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"WST", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"XAF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"XAG", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XAU", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XBA", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XBB", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XBC", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XBD", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XCD", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"XDR", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XEU", UCURR_UNCOMMON|UCURR_DEPRECATED},
    {u8"XFO", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XFU", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XOF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"XPD", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XPF", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"XPT", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XRE", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XSU", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XTS", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XUA", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"XXX", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"YDD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"YER", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"YUD", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"YUM", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"YUN", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"YUR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ZAL", UCURR_UNCOMMON|UCURR_NON_DEPRECATED},
    {u8"ZAR", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ZMK", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ZMW", UCURR_COMMON|UCURR_NON_DEPRECATED},
    {u8"ZRN", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ZRZ", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ZWL", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ZWR", UCURR_COMMON|UCURR_DEPRECATED},
    {u8"ZWD", UCURR_COMMON|UCURR_DEPRECATED},
        ...
        for (int32_t i=0; i<ures_getSize(currencyMapArray); i++) {
            ...
            if (U_SUCCESS(localStatus)) {
                for (int32_t j=0; j<ures_getSize(currencyArray); j++) {
                    ...
                    // get the ISO code
                    int32_t isoLength = 0;
                    UResourceBundle *idRes = ures_getByKey(currencyRes, u8"id", NULL, &localStatus);
                    if (idRes == NULL) {
                        continue;
                    }
                    ...
                    UResourceBundle *fromRes = ures_getByKey(currencyRes, u8"from", NULL, &localStatus);
                    ...
                    UResourceBundle *toRes = ures_getByKey(currencyRes, u8"to", NULL, &localStatus);
                 ...
                 UErrorCode* ec)
{
    ...
    if (ec != NULL && U_SUCCESS(*ec))
    {
        ...
        UErrorCode localStatus = U_ZERO_ERROR;
        char id[ULOC_FULLNAME_CAPACITY];
        uloc_getKeywordValue(locale, u8"currency", id, ULOC_FULLNAME_CAPACITY, &localStatus);
        // get country or country_variant in `id'
        /*uint32_t variantType =*/ idForLocale(locale, id, sizeof(id), ec);
                ...
                UErrorCode* ec)
{
    ...
    if (ec != NULL && U_SUCCESS(*ec))
    {
        ...
        if ((buff && buffCapacity) || !buffCapacity )
        {
            ...
            UErrorCode localStatus = U_ZERO_ERROR;
            char id[ULOC_FULLNAME_CAPACITY];
            resLen = uloc_getKeywordValue(locale, u8"currency", id, ULOC_FULLNAME_CAPACITY, &localStatus);
            ...
            if (U_SUCCESS(localStatus))
            {
                ...
                for (int32_t i=0; i<ures_getSize(countryArray); i++)
                {
                    // get the currency resource
                    UResourceBundle *currencyRes = ures_getByIndex(countryArray, i, NULL, &localStatus);
                    s = ures_getStringByKey(currencyRes, u8"id", &resLen, &localStatus);
...
U_CAPI UEnumeration *U_EXPORT2 ucurr_getKeywordValuesForLocale(const char *key, const char *locale, UBool commonlyUsed, UErrorCode* status) {
    ...
    UResourceBundle *bundle = ures_openDirect(U_ICUDATA_CURR, u8"supplementalData", status);
    ures_getByKey(bundle, u8"CurrencyMap", bundle, status);
    ...
    while (U_SUCCESS(*status) && ures_hasNext(bundle)) {
        ...
        while (U_SUCCESS(*status) && ures_hasNext(&regbndl)) {
            ...
            ures_getUTF8StringByKey(&curbndl, u8"id", curID, &curIDLength, TRUE, status);
            /* optimize - use the utf-8 string */
#else
            {
                       const UChar* defString = ures_getStringByKey(&curbndl, u8"id", &curIDLength, status);
                       if(U_SUCCESS(*status)) {
            ...
            UBool hasTo = FALSE;
            ures_getByKey(&curbndl, u8"to", &to, status);
            if (U_FAILURE(*status)) {
                // Do nothing here...
                *status = U_ZERO_ERROR;
            } else {
    ...
    if (U_SUCCESS(*status)) {
        if (commonlyUsed) {
            if (ulist_getListSize(values) == 0) {
                ...
                uenum_close(en);
                en = ucurr_getKeywordValuesForLocale(key, u8"und", TRUE, status);
            }
        } else {
...
ucurr_getNumericCode(const UChar* currency) {
    ...
    if (currency && u_strlen(currency) == ISO_CURRENCY_CODE_LENGTH) {
        ...
        UResourceBundle *bundle = ures_openDirect(0, u8"currencyNumericCodes", &status);
        ures_getByKey(bundle, u8"codeMap", bundle, &status);
        if (U_SUCCESS(status)) {
