...
#define IN_COMMON_MODE(mode) (mode == '\x61' || mode == '\x63')
#define IN_DLL_MODE(mode)    (mode == '\x64' || mode == '\x6c')
#define IN_STATIC_MODE(mode) (mode == '\x73')
#define IN_FILES_MODE(mode)  (mode == '\x66')
...
static struct {
    ...
    const char *desc;
        ...
        { u8"files", 0,           u8"Uses raw data files (no effect). Installation copies all files to the target location." },
#if U_PLATFORM_HAS_WIN32_API
        { u8"dll",    u8"library",  u8"Generates one common data file and one shared library, <package>.dll"},
        { u8"common", u8"archive",  u8"Generates just the common file, <package>.dat"},
        { u8"static", u8"static",   u8"Generates one statically linked library, " LIB_PREFIX u8"<package>" UDATA_LIB_SUFFIX }
#else
#ifdef UDATA_SO_SUFFIX
        { u8"dll",    u8"library",  u8"Generates one shared library, <package>" UDATA_SO_SUFFIX },
#endif
        { u8"common", u8"archive",  u8"Generates one common data file, <package>.dat" },
        { u8"static", u8"static",   u8"Generates one statically linked library, " LIB_PREFIX u8"<package>" UDATA_LIB_SUFFIX }
#endif
};
...
static UOption options[]={
    ...
    /*01*/    UOPTION_DEF( u8"bldopt",  '\x4f', UOPT_REQUIRES_ARG), /* on Win32 it is release or debug */
    /*02*/    UOPTION_DEF( u8"mode",    '\x6d', UOPT_REQUIRES_ARG),
    /*03*/    UOPTION_HELP_H,                                   /* -h */
    /*04*/    UOPTION_HELP_QUESTION_MARK,                       /* -? */
    /*05*/    UOPTION_VERBOSE,                                  /* -v */
    /*06*/    UOPTION_COPYRIGHT,                                /* -c */
    /*07*/    UOPTION_DEF( u8"comment", '\x43', UOPT_REQUIRES_ARG),
    /*08*/    UOPTION_DESTDIR,                                  /* -d */
    /*11*/    UOPTION_DEF( u8"rebuild", '\x46', UOPT_NO_ARG),
    /*12*/    UOPTION_DEF( u8"tempdir", '\x54', UOPT_REQUIRES_ARG),
    /*13*/    UOPTION_DEF( u8"install", '\x49', UOPT_REQUIRES_ARG),
    /*14*/    UOPTION_SOURCEDIR ,
    /*15*/    UOPTION_DEF( u8"entrypoint", '\x65', UOPT_REQUIRES_ARG),
    /*16*/    UOPTION_DEF( u8"revision", '\x72', UOPT_REQUIRES_ARG),
    /*17*/    UOPTION_DEF( u8"force-prefix", '\x66', UOPT_NO_ARG),
    /*18*/    UOPTION_DEF( u8"libname", '\x4c', UOPT_REQUIRES_ARG),
    /*19*/    UOPTION_DEF( u8"quiet", '\x71', UOPT_NO_ARG),
    /*20*/    UOPTION_DEF( u8"without-assembly", '\x77', UOPT_NO_ARG),
    /*21*/    UOPTION_DEF(u8"zos-pds-build", '\x7a', UOPT_NO_ARG),
    /*22*/    UOPTION_DEF(u8"windows-uwp-build", '\x75', UOPT_NO_ARG),
    /*23*/    UOPTION_DEF(u8"windows-uwp-arm-build", '\x61', UOPT_NO_ARG)
};
...
const char options_help[][320]={
    u8"Set the data name",
#ifdef U_MAKE_IS_NMAKE
    u8"The directory where the ICU is located (e.g. <ICUROOT> which contains the bin directory)",
#else
    u8"Specify options for the builder.",
#endif
    u8"Specify the mode of building (see below; default: common)",
    u8"This usage text",
    u8"This usage text",
    u8"Make the output verbose",
    u8"Use the standard ICU copyright",
    u8"Use a custom comment (instead of the copyright)",
    u8"Specify the destination directory for files",
    u8"Force rebuilding of all data",
    u8"Specify temporary dir (default: output dir)",
    u8"Install the data (specify target)",
    u8"Specify a custom source directory",
    u8"Specify a custom entrypoint name (default: short name)",
    u8"Specify a version when packaging in dll or static mode",
    u8"Add package to all file names if not present",
    u8"Library name to build (if different than package name)",
    u8"Quiet mode. (e.g. Do not output a readme file for static libraries)",
    u8"Build the data without assembly code",
    u8"Build PDS dataset (zOS build only)",
    u8"Build for Universal Windows Platform (Windows build only)",
    u8"Set DLL machine type for UWP to target windows ARM (Windows UWP build only)"
};

const char  *progname = u8"PKGDATA";
...
main(int argc, char* argv[]) {
    ...
    if(options[HELP].doesOccur || options[HELP_QUESTION_MARK].doesOccur) {
        ...
        if(!needsHelp && argc<0) {
            fprintf(stderr,
                u8"%s: error in command line argument \"%s\"\n",
                progname,
                argv[-argc]);
            fprintf(stderr, u8"Run '%s --help' for help.\n", progname);
            return 1;
        }


#if !defined(WINDOWS_WITH_MSVC) || defined(USING_CYGWIN)
        if(!options[BLDOPT].doesOccur && uprv_strcmp(options[MODE].value, u8"common") != 0) {
          if (pkg_getOptionsFromICUConfig(options[VERBOSE].doesOccur, &options[BLDOPT]) != 0) {
                fprintf(stderr, u8" required parameter is missing: -O is required for static and shared builds.\n");
                fprintf(stderr, u8"Run '%s --help' for help.\n", progname);
                return 1;
            }
        }
        ...
        if(options[BLDOPT].doesOccur) {
            fprintf(stdout, u8"Warning: You are using the -O option which is not needed for MSVC build on Windows.\n");
        }
#endif

        if(!options[NAME].doesOccur) /* -O we already have - don't report it. */
        {
            fprintf(stderr, u8" required parameter -p is missing \n");
            fprintf(stderr, u8"Run '%s --help' for help.\n", progname);
            return 1;
        }
        ...
        if(argc == 1) {
            fprintf(stderr,
                u8"No input files specified.\n"
                u8"Run '%s --help' for help.\n", progname);
            return 1;
        }
    }   /* end !needsHelp */
    ...
    if(argc<0 || needsHelp  ) {
        fprintf(stderr,
            u8"usage: %s [-options] [-] [packageFile] \n"
            u8"\tProduce packaged ICU data from the given list(s) of files.\n"
            u8"\t'-' by itself means to read from stdin.\n"
            u8"\tpackageFile is a text file containing the list of files to package.\n",
            progname);

        fprintf(stderr, u8"\n options:\n");
        for(i=0;i<UPRV_LENGTHOF(options);i++) {
            fprintf(stderr, u8"%-5s -%c %s%-10s  %s\n",
                (i<1?u8"[REQ]":u8""),
                options[i].shortName,
                options[i].longName ? u8"or --" : u8"     ",
                options[i].longName ? options[i].longName : u8"",
                options_help[i]);
        }

        fprintf(stderr, u8"modes: (-m option)\n");
        for(i=0;i<UPRV_LENGTHOF(modes);i++) {
            fprintf(stderr, u8"   %-9s ", modes[i].name);
            if (modes[i].alt_name) {
                fprintf(stderr, u8"/ %-9s", modes[i].alt_name);
            } else {
                fprintf(stderr, u8"           ");
            }
            fprintf(stderr, u8"  %s\n", modes[i].desc);
    ...
    o.shortName = options[NAME].value;
    {
        ...
        if (*(sp = o.shortName)) {
            *cp++ = isalpha(*sp) ? * sp : '\x5f';
            for (++sp; *sp; ++sp) {
                *cp++ = isalnum(*sp) ? *sp : '\x5f';
            }
        }
        *cp = 0;
...
#else
      o.pdsbuild = FALSE;
      fprintf(stdout, u8"Warning: You are using the -z option which only works on z/OS.\n");
    ...
    if (options[WITHOUT_ASSEMBLY].doesOccur) {
        ...
        fprintf(stdout, u8"Warning: You are using the option to build without assembly code which is not supported on this platform.\n");
        fprintf(stdout, u8"Warning: This option will be ignored.\n");
#else
    ...
    loadLists(&o, &status);
    if( U_FAILURE(status) ) {
        fprintf(stderr, u8"error loading input file lists: %s\n", u_errorName(status));
        return 2;
    }
...
static int runCommand(const char* command, UBool specialHandling) {
    ...
    if (!specialHandling) {
        ...
        sprintf(cmd, u8"bash -c \"%s\"", command);

#elif U_PLATFORM == U_PF_OS400
        sprintf(cmd, u8"QSH CMD('%s')", command);
#endif
#else
...
normal_command_mode:
    ...
    if (result != 0) {
        fprintf(stderr, u8"-- return status = %d\n", result);
    }
...
#define LN_CMD u8"ln -s"
#define RM_CMD u8"rm -f"

static int32_t pkg_executeOptions(UPKGOptions *o) {
    int32_t result = 0;

    const char mode = o->mode[0];
    char targetDir[SMALL_BUFFER_MAX_SIZE] = u8"";
    char tmpDir[SMALL_BUFFER_MAX_SIZE] = u8"";
    char datFileName[SMALL_BUFFER_MAX_SIZE] = u8"";
    char datFileNamePath[LARGE_BUFFER_MAX_SIZE] = u8"";
    char checkLibFile[LARGE_BUFFER_MAX_SIZE] = u8"";
    ...
    if (IN_FILES_MODE(mode)) {
        ...
        if (o->install != NULL) {
            ...
            if(o->verbose) {
              fprintf(stdout, u8"# Install: Files mode, copying files to %s..\n", targetDir);
            }
        ...
        if(o->verbose) {
          fprintf(stdout, u8"# Writing package file %s ..\n", datFileNamePath);
        }
        result = writePackageDatFile(datFileNamePath, o->comment, o->srcDir, o->fileListFiles->str, NULL, U_CHARSET_FAMILY ? '\x65' :  U_IS_BIG_ENDIAN ? '\x62' : '\x6c');
        if (result != 0) {
            fprintf(stderr,u8"Error writing package dat file.\n");
            return result;
        }

        if (IN_COMMON_MODE(mode)) {
            char targetFileNamePath[LARGE_BUFFER_MAX_SIZE] = u8"";
            ...
            if (uprv_strcmp(datFileNamePath, targetFileNamePath) != 0) {
                if (T_FileStream_file_exists(targetFileNamePath)) {
                    if ((result = remove(targetFileNamePath)) != 0) {
                        fprintf(stderr, u8"Unable to remove old dat file: %s\n",
                                targetFileNamePath);
                        return result;
                    }
                }
                ...
                if (o->verbose) {
                    fprintf(stdout, u8"# Moving package file to %s ..\n",
                            targetFileNamePath);
                }
                if (result != 0) {
                    fprintf(
                            stderr,
                            u8"Unable to move dat file (%s) to target location (%s).\n",
            ...
            char gencFilePath[SMALL_BUFFER_MAX_SIZE] = u8"";
            char version_major[10] = u8"";
            ...
            if (o->version != NULL) {
                ...
                noVersion = TRUE;
                if (IN_DLL_MODE(mode)) {
                    fprintf(stdout, u8"Warning: Providing a revision number with the -r option is recommended when packaging data in the current mode.\n");
                }
            }
            ...
            createFileNames(o, mode, version_major, o->version == NULL ? u8"" : o->version, o->libName, reverseExt, noVersion);
            ...
            if ((o->version!=NULL || IN_STATIC_MODE(mode)) && o->rebuild == FALSE && o->pdsbuild == FALSE) {
                ...
                sprintf(checkLibFile, u8"%s%s", targetDir, libFileNames[LIB_FILE_VERSION]);
                if (T_FileStream_file_exists(checkLibFile)) {
                    if (isFileModTimeLater(checkLibFile, o->srcDir, TRUE) && isFileModTimeLater(checkLibFile, o->options)) {
                        if (o->install != NULL) {
                          if(o->verbose) {
                            fprintf(stdout, u8"# Installing already-built library into %s\n", o->install);
                          }
                          result = pkg_installLibrary(o->install, targetDir, noVersion);
                        } else {
                          if(o->verbose) {
                            printf(u8"# Not rebuilding %s - up to date.\n", checkLibFile);
                          }
                        }
                        return result;
                    } else if (o->verbose && (o->install!=NULL)) {
                      fprintf(stdout, u8"# Not installing up-to-date library %s into %s\n", checkLibFile, o->install);
                    }
                } else if(o->verbose && (o->install!=NULL)) {
                  fprintf(stdout, u8"# Not installing missing %s into %s\n", checkLibFile, o->install);
                }
            }
            ...
            if (!o->withoutAssembly && pkgDataFlags[GENCCODE_ASSEMBLY_TYPE][0] != 0) {
                ...
                if(o->verbose) {
                  fprintf(stdout, u8"# Generating assembly code %s of type %s ..\n", gencFilePath, genccodeAssembly);
                }
                ...
                if (genccodeAssembly &&
                    ...
                    if (result != 0) {
                        fprintf(stderr, u8"Error generating assembly code for data.\n");
                        return result;
                    } else if (IN_STATIC_MODE(mode)) {
                      if(o->install != NULL) {
                        if(o->verbose) {
                          fprintf(stdout, u8"# Installing static library into %s\n", o->install);
                        }
                        result = pkg_installLibrary(o->install, targetDir, noVersion);
                      }
                      return result;
                    ...
                    fprintf(stderr,u8"Assembly type \"%s\" is unknown.\n", genccodeAssembly);
                    return -1;
                }
            } else {
                if(o->verbose) {
                  fprintf(stdout, u8"# Writing object code to %s ..\n", gencFilePath);
                }
                if (o->withoutAssembly) {
                    ...
                    fprintf(stderr, u8"Error- BUILD_DATA_WITHOUT_ASSEMBLY is not defined. Internal error.\n");
#endif
                } else {
#ifdef CAN_WRITE_OBJ_CODE
                    ...
                    result = pkg_createWithoutAssemblyCode(o, targetDir, mode);
#else
                    fprintf(stderr, u8"Error- neither CAN_WRITE_OBJ_CODE nor BUILD_DATA_WITHOUT_ASSEMBLY are defined. Internal error.\n");
                    return 1;
#endif
                }

                if (result != 0) {
                    fprintf(stderr, u8"Error generating package data.\n");
                    return result;
                }
            }
            ...
            if(!IN_STATIC_MODE(mode)) {
                ...
                if(o->verbose) {
                  fprintf(stdout, u8"# Creating data archive library file ..\n");
                }
                result = pkg_archiveLibrary(targetDir, o->version, reverseExt);
                if (result != 0) {
                    fprintf(stderr, u8"Error creating data archive library file.\n");
                   return result;
                }
#if U_PLATFORM != U_PF_OS400
                if (!noVersion) {
...
#endif
                    if (result != 0) {
                        fprintf(stderr, u8"Error creating symbolic links of the data library file.\n");
                        return result;
                    }
                }
#endif
            ...
            if (o->install != NULL) {
                if(o->verbose) {
                  fprintf(stdout, u8"# Installing library file to %s ..\n", o->install);
                }
                result = pkg_installLibrary(o->install, targetDir, noVersion);
                if (result != 0) {
                    fprintf(stderr, u8"Error installing the data library.\n");
                    return result;
                }
            }
#endif
...
static int32_t initializePkgDataFlags(UPKGOptions *o) {
    ...
    do {
        ...
        if (pkgDataFlags != NULL) {
            for (int32_t i = 0; i < PKGDATA_FLAGS_SIZE; i++) {
                ...
                if (pkgDataFlags[i] != NULL) {
                    ...
                    fprintf(stderr,u8"Error allocating memory for pkgDataFlags.\n");
        ...
        if(o->verbose) {
          fprintf(stdout, u8"# Reading options file %s\n", o->options);
        }
        status = U_ZERO_ERROR;
        ...
        if (status == U_BUFFER_OVERFLOW_ERROR) {
            ...
            fprintf(stderr,u8"Unable to open or read \"%s\" option file. status = %s\n", o->options, u_errorName(status));
            return -1;
        }
#endif
        if(o->verbose) {
            fprintf(stdout, u8"# pkgDataFlags=\n");
            for(int32_t i=0;i<PKGDATA_FLAGS_SIZE;i++) {
                fprintf(stdout, u8"  [%d] %s:  %s\n", i, FLAG_NAMES[i], pkgDataFlags[i]);
            }
            fprintf(stdout, u8"\n");
        }
#if !defined(WINDOWS_WITH_MSVC) || defined(USING_CYGWIN)
...
static void createFileNames(UPKGOptions *o, const char mode, const char *version_major, const char *version, const char *libName, UBool reverseExt, UBool noVersion) {
    const char* FILE_EXTENSION_SEP = uprv_strlen(pkgDataFlags[SO_EXT]) == 0 ? u8"" : u8".";
    const char* FILE_SUFFIX = pkgDataFlags[LIB_EXT_ORDER][0] == '\x2e' ? u8"." : u8"";

#if U_PLATFORM == U_PF_MINGW
        /* MinGW does not need the library prefix when building in dll mode. */
        if (IN_DLL_MODE(mode)) {
            sprintf(libFileNames[LIB_FILE], u8"%s", libName);
        } else {
            sprintf(libFileNames[LIB_FILE], u8"%s%s",
        ...
        if(o->verbose) {
          fprintf(stdout, u8"# libFileName[LIB_FILE] = %s\n", libFileNames[LIB_FILE]);
        }

#if U_PLATFORM == U_PF_MINGW
        // Name the import library lib*.dll.a
        sprintf(libFileNames[LIB_FILE_MINGW], u8"lib%s.dll.a", libName);
#elif U_PLATFORM == U_PF_CYGWIN
        sprintf(libFileNames[LIB_FILE_CYGWIN], u8"cyg%s%s%s",
                libName,
                FILE_EXTENSION_SEP,
                pkgDataFlags[SO_EXT]);
        sprintf(libFileNames[LIB_FILE_CYGWIN_VERSION], u8"cyg%s%s%s%s",
                libName,
                version_major,
                FILE_EXTENSION_SEP,
                pkgDataFlags[SO_EXT]);

        uprv_strcat(pkgDataFlags[SO_EXT], u8".");
        uprv_strcat(pkgDataFlags[SO_EXT], pkgDataFlags[A_EXT]);
#elif U_PLATFORM == U_PF_OS400 || defined(_AIX)
        sprintf(libFileNames[LIB_FILE_VERSION_TMP], u8"%s%s%s",
                libFileNames[LIB_FILE],
                FILE_EXTENSION_SEP,
                pkgDataFlags[SOBJ_EXT]);
#elif U_PLATFORM == U_PF_OS390
        sprintf(libFileNames[LIB_FILE_VERSION_TMP], u8"%s%s%s%s%s",
                    libFileNames[LIB_FILE],
                    pkgDataFlags[LIB_EXT_ORDER][0] == '\x2e' ? u8"." : u8"",
                    reverseExt ? version : pkgDataFlags[SOBJ_EXT],
                    FILE_EXTENSION_SEP,
                    reverseExt ? pkgDataFlags[SOBJ_EXT] : version);

        sprintf(libFileNames[LIB_FILE_OS390BATCH_VERSION], u8"%s%s.x",
                    libFileNames[LIB_FILE],
                    version);
        sprintf(libFileNames[LIB_FILE_OS390BATCH_MAJOR], u8"%s%s.x",
                    libFileNames[LIB_FILE],
                    version_major);
#else
...
#endif
        if (noVersion && !reverseExt) {
            sprintf(libFileNames[LIB_FILE_VERSION_MAJOR], u8"%s%s%s",
                    libFileNames[LIB_FILE],
                    FILE_SUFFIX,
                    pkgDataFlags[SO_EXT]);

            sprintf(libFileNames[LIB_FILE_VERSION], u8"%s%s%s",
                    libFileNames[LIB_FILE],
                    FILE_SUFFIX,
                    pkgDataFlags[SO_EXT]);
        } else {
            sprintf(libFileNames[LIB_FILE_VERSION_MAJOR], u8"%s%s%s%s%s",
                    libFileNames[LIB_FILE],
                    FILE_SUFFIX,
            ...
            sprintf(libFileNames[LIB_FILE_VERSION], u8"%s%s%s%s%s",
                    libFileNames[LIB_FILE],
                    FILE_SUFFIX,
        ...
        if(o->verbose) {
          fprintf(stdout, u8"# libFileName[LIB_FILE_VERSION] = %s\n", libFileNames[LIB_FILE_VERSION]);
        }
        ...
        if(IN_STATIC_MODE(mode)) {
            sprintf(libFileNames[LIB_FILE_VERSION], u8"%s.%s", libFileNames[LIB_FILE], pkgDataFlags[A_EXT]);
            libFileNames[LIB_FILE_VERSION_MAJOR][0]=0;
            if(o->verbose) {
              fprintf(stdout, u8"# libFileName[LIB_FILE_VERSION] = %s  (static)\n", libFileNames[LIB_FILE_VERSION]);
            }
        }
}
...
static int32_t pkg_createSymLinks(const char *targetDir, UBool specialHandling) {
    ...
    if (result != 0) {
        fprintf(stderr, u8"Error creating symbolic links. Failed command: %s\n", cmd);
        return result;
    }
#endif
    ...
    if (specialHandling) {
#if U_PLATFORM == U_PF_CYGWIN
        sprintf(name1, u8"%s", libFileNames[LIB_FILE_CYGWIN]);
        sprintf(name2, u8"%s", libFileNames[LIB_FILE_CYGWIN_VERSION]);
#elif U_PLATFORM == U_PF_OS390
        /* Create the symbolic links for the import data */
        /* Use the cmd buffer to store path to import data file to check its existence */
        sprintf(cmd, u8"%s/%s", targetDir, libFileNames[LIB_FILE_OS390BATCH_VERSION]);
        if (T_FileStream_file_exists(cmd)) {
            ...
            sprintf(cmd, u8"cd %s && %s %s.x && %s %s %s.x",
                    targetDir,
                    RM_CMD,
                    libFileNames[LIB_FILE],
                    LN_CMD,
        ...
        sprintf(name1, u8"%s%s%s", libFileNames[LIB_FILE], FILE_EXTENSION_SEP, pkgDataFlags[SO_EXT]);
        sprintf(name2, u8"%s", libFileNames[LIB_FILE_VERSION]);
#else
...
static int32_t pkg_installLibrary(const char *installDir, const char *targetDir, UBool noVersion) {
    ...
    if (result != 0) {
        fprintf(stderr, u8"Error installing library. Failed command: %s\n", cmd);
        return result;
    }

#ifdef CYGWINMSVC
    sprintf(cmd, u8"cd %s && %s %s.lib %s",
            targetDir,
            pkgDataFlags[INSTALL_CMD],
            libFileNames[LIB_FILE],
            installDir
...
static int32_t pkg_installCommonMode(const char *installDir, const char *fileName) {
    int32_t result = 0;
    char cmd[SMALL_BUFFER_MAX_SIZE] = u8"";
    ...
    if (!T_FileStream_file_exists(installDir)) {
        ...
        if (U_FAILURE(status)) {
            fprintf(stderr, u8"Error creating installation directory: %s\n", installDir);
            return -1;
        }
    }
#ifndef U_WINDOWS_WITH_MSVC
    sprintf(cmd, u8"%s %s %s", pkgDataFlags[INSTALL_CMD], fileName, installDir);
#else
    sprintf(cmd, u8"%s %s %s %s", WIN_INSTALL_CMD, fileName, installDir, WIN_INSTALL_CMD_FLAGS);
#endif

    result = runCommand(cmd);
    if (result != 0) {
        fprintf(stderr, u8"Failed to install data file with command: %s\n", cmd);
    }
...
#define WIN_INSTALL_CMD u8"xcopy"
#define WIN_INSTALL_CMD_FLAGS u8"/E /Y /K"
#endif
static int32_t pkg_installFileMode(const char *installDir, const char *srcDir, const char *fileListName) {
    int32_t result = 0;
    char cmd[SMALL_BUFFER_MAX_SIZE] = u8"";
...
#ifndef U_WINDOWS_WITH_MSVC
    char buffer[SMALL_BUFFER_MAX_SIZE] = u8"";
    int32_t bufferLength = 0;

    FileStream *f = T_FileStream_open(fileListName, u8"r");
    if (f != NULL) {
        for(;;) {
            if (T_FileStream_readLine(f, buffer, SMALL_BUFFER_MAX_SIZE) != NULL) {
                ...
                if (!T_FileStream_eof(f)) {
                    fprintf(stderr, u8"Failed to read line from file: %s\n", fileListName);
                    result = -1;
                }
                break;
            }
        ...
        result = -1;
        fprintf(stderr, u8"Unable to open list file: %s\n", fileListName);
    }
#else
    sprintf(cmd, u8"%s %s %s %s", WIN_INSTALL_CMD, srcDir, installDir, WIN_INSTALL_CMD_FLAGS);
    result = runCommand(cmd);
    if (result != 0) {
        fprintf(stderr, u8"Failed to install data file with command: %s\n", cmd);
    }
...
static int32_t pkg_archiveLibrary(const char *targetDir, const char *version, UBool reverseExt) {
    ...
    if (uprv_strcmp(pkgDataFlags[SOBJ_EXT], pkgDataFlags[SO_EXT]) != 0 && uprv_strcmp(pkgDataFlags[A_EXT], pkgDataFlags[SO_EXT]) == 0) {
        sprintf(libFileNames[LIB_FILE_VERSION], u8"%s%s%s.%s",
                libFileNames[LIB_FILE],
                pkgDataFlags[LIB_EXT_ORDER][0] == '\x2e' ? u8"." : u8"",
                reverseExt ? version : pkgDataFlags[SO_EXT],
                reverseExt ? pkgDataFlags[SO_EXT] : version);

        sprintf(cmd, u8"%s %s %s%s %s%s",
                pkgDataFlags[AR],
        ...
        result = runCommand(cmd);
        if (result != 0) {
            fprintf(stderr, u8"Error creating archive library. Failed command: %s\n", cmd);
            return result;
        }
...
static int32_t pkg_generateLibraryFile(const char *targetDir, const char mode, const char *objectFile, char *command, UBool specialHandling) {
    ...
    if (IN_STATIC_MODE(mode)) {
        if (cmd == NULL) {
            ...
            if ((cmd = (char *)uprv_malloc(sizeof(char) * length)) == NULL) {
                fprintf(stderr, u8"Unable to allocate memory for command.\n");
                return -1;
            }
            freeCmd = TRUE;
        }
        sprintf(cmd, u8"%s %s %s%s %s",
                pkgDataFlags[AR],
                pkgDataFlags[ARFLAGS],
                targetDir,
                libFileNames[LIB_FILE_VERSION],
        ...
        if (cmd == NULL) {
            length = uprv_strlen(pkgDataFlags[GENLIB]) + uprv_strlen(pkgDataFlags[LDICUDTFLAGS]) +
                     ...
                     uprv_strlen(pkgDataFlags[LD_SONAME][0] == 0 ? u8"" : libFileNames[LIB_FILE_VERSION_MAJOR]) +
...
#if U_PLATFORM == U_PF_MINGW
        sprintf(cmd, u8"%s%s%s %s -o %s%s %s %s%s %s %s",
                pkgDataFlags[GENLIB],
                targetDir,
        ...
        sprintf(cmd, u8"%s %s%s;%s %s -o %s%s %s %s%s %s %s",
                RM_CMD,
                targetDir,
                libFileNames[LIB_FILE_VERSION_TMP],
                pkgDataFlags[GENLIB],
                ...
                libFileNames[LIB_FILE_VERSION_TMP],
#else
        sprintf(cmd, u8"%s %s -o %s%s %s %s%s %s %s",
                pkgDataFlags[GENLIB],
                pkgDataFlags[LDICUDTFLAGS],
                targetDir,
                libFileNames[LIB_FILE_VERSION_TMP],
                ...
                pkgDataFlags[LD_SONAME][0] == 0 ? u8"" : libFileNames[LIB_FILE_VERSION_MAJOR],
        ...
        PDS_Name[0] = 0;
        PDS_LibName[0] = 0;
        if (specialHandling && uprv_strcmp(libFileNames[LIB_FILE],u8"libicudata") == 0) {
            if (env_tmp = getenv(u8"ICU_PDS_NAME")) {
                sprintf(PDS_Name, u8"%s%s",
                        env_tmp,
                        u8"DA");
                strcat(PDS_Name, getenv(u8"ICU_PDS_NAME_SUFFIX"));
            } else if (env_tmp = getenv(u8"PDS_NAME_PREFIX")) {
                sprintf(PDS_Name, u8"%s%s",
                        env_tmp,
                        U_ICU_VERSION_SHORT u8"DA");
            } else {
                sprintf(PDS_Name, u8"%s%s",
                        u8"IXMI",
                        U_ICU_VERSION_SHORT u8"DA");
            }
        } else if (!specialHandling && uprv_strcmp(libFileNames[LIB_FILE],u8"libicudata_stub") == 0) {
            if (env_tmp = getenv(u8"ICU_PDS_NAME")) {
                sprintf(PDS_Name, u8"%s%s",
                        env_tmp,
                        u8"D1");
        ...
        if (PDS_Name[0]) {
            sprintf(PDS_LibName,u8"%s%s%s%s%s",
                    u8"\"//'",
                    getenv(u8"LOADMOD"),
                    u8"(",
                    PDS_Name,
                    u8")'\"");
            sprintf(cmd, u8"%s %s -o %s %s %s%s %s %s",
                   pkgDataFlags[GENLIB],
                   pkgDataFlags[LDICUDTFLAGS],
                   PDS_LibName,
                   objectFile,
    ...
    if (result != 0) {
        fprintf(stderr, u8"Error generating library file. Failed command: %s\n", cmd);
    }
...
static int32_t pkg_createWithAssemblyCode(const char *targetDir, const char mode, const char *gencFilePath) {
    char tempObjectFile[SMALL_BUFFER_MAX_SIZE] = u8"";
    char *cmd;
    int32_t result = 0;
    ...
    tempObjectFile[uprv_strlen(tempObjectFile)-1] = '\x6f';
    ...
    uprv_free(cmd);
    if (result != 0) {
        fprintf(stderr, u8"Error creating with assembly code. Failed command: %s\n", cmd);
        return result;
    }
...
static int32_t pkg_createWithoutAssemblyCode(UPKGOptions *o, const char *targetDir, const char mode) {
    ...
    char *buffer;
    char *cmd;
    char gencmnFile[SMALL_BUFFER_MAX_SIZE] = u8"";
    char tempObjectFile[SMALL_BUFFER_MAX_SIZE] = u8"";
#ifdef USE_SINGLE_CCODE_FILE
    char icudtAll[SMALL_BUFFER_MAX_SIZE] = u8"";
    FileStream *icudtAllFile = NULL;

    sprintf(icudtAll, u8"%s%s%sall.c",
            o->tmpDir,
            PKGDATA_FILE_SEP_STRING,
            libFileNames[LIB_FILE]);
    /* Remove previous icudtall.c file. */
    if (T_FileStream_file_exists(icudtAll) && (result = remove(icudtAll)) != 0) {
        fprintf(stderr, u8"Unable to remove old icudtall file: %s\n", icudtAll);
        return result;
    }

    if((icudtAllFile = T_FileStream_open(icudtAll, u8"w"))==NULL) {
        fprintf(stderr, u8"Unable to write to icudtall file: %s\n", icudtAll);
        return result;
    }
#endif
    ...
    if ((cmd = (char *)uprv_malloc((listSize + 2) * SMALL_BUFFER_MAX_SIZE)) == NULL) {
        fprintf(stderr, u8"Unable to allocate memory for cmd.\n");
        return -1;
    } else if ((buffer = (char *)uprv_malloc((listSize + 1) * SMALL_BUFFER_MAX_SIZE)) == NULL) {
        fprintf(stderr, u8"Unable to allocate memory for buffer.\n");
        uprv_free(cmd);
        return -1;
    }
    ...
    for (int32_t i = 0; i < (listSize + 1); i++) {
        ...
        if (i == 0) {
            ...
            sprintf(buffer, u8"%s",tempObjectFile);
#endif
        } else {
            ...
            for (int32_t n = 0; n < DATA_PREFIX_LENGTH; n++) {
                ...
                if (pSubstring != NULL) {
                    ...
                    sprintf(newName, u8"%s_%s",
                            DATA_PREFIX[n],
                            newNameTmp);
                    sprintf(dataName, u8"%s_%s",
                            o->shortName,
                            DATA_PREFIX[n]);
                }
                if (newName[0] != 0) {
        ...
        result = runCommand(cmd);
        if (result != 0) {
            fprintf(stderr, u8"Error creating library without assembly code. Failed command: %s\n", cmd);
            break;
        }

        uprv_strcat(buffer, u8" ");
...
#ifdef WINDOWS_WITH_MSVC
#define LINK_CMD u8"link.exe /nologo /release /out:"
#define LINK_FLAGS u8"/DLL /NOENTRY /MANIFEST:NO /implib:"
#ifdef _WIN64
#define LINK_EXTRA_UWP_FLAGS u8"/NXCOMPAT /DYNAMICBASE /APPCONTAINER "
#else
#define LINK_EXTRA_UWP_FLAGS u8"/NXCOMPAT /SAFESEH /DYNAMICBASE /APPCONTAINER /MACHINE:X86"
#endif
#define LINK_EXTRA_UWP_FLAGS_ARM u8"/NXCOMPAT /DYNAMICBASE /APPCONTAINER /MACHINE:ARM"
#define LINK_EXTRA_NO_UWP_FLAGS u8"/base:0x4ad00000 "
#define LIB_CMD u8"LIB.exe /nologo /out:"
#define LIB_FILE u8"icudt.lib"
...
static int32_t pkg_createWindowsDLL(const char mode, const char *gencFilePath, UPKGOptions *o) {
    ...
    char cmd[LARGE_BUFFER_MAX_SIZE];
    if (IN_STATIC_MODE(mode)) {
        char staticLibFilePath[SMALL_BUFFER_MAX_SIZE] = u8"";

#ifdef CYGWINMSVC
        sprintf(staticLibFilePath, u8"%s%s%s%s%s",
                o->targetDir,
                PKGDATA_FILE_SEP_STRING,
                pkgDataFlags[LIBPREFIX],
                o->libName,
        ...
        sprintf(staticLibFilePath, u8"%s%s%s%s%s",
                o->targetDir,
                PKGDATA_FILE_SEP_STRING,
                (strstr(o->libName, u8"icudt") ? u8"s" : u8""),
                o->libName,
                LIB_EXT);
#endif
        ...
        char dllFilePath[SMALL_BUFFER_MAX_SIZE] = u8"";
        char libFilePath[SMALL_BUFFER_MAX_SIZE] = u8"";
        char resFilePath[SMALL_BUFFER_MAX_SIZE] = u8"";
        char tmpResFilePath[SMALL_BUFFER_MAX_SIZE] = u8"";
        ...
        if (T_FileStream_file_exists(tmpResFilePath)) {
            sprintf(resFilePath, u8"\"%s\"", tmpResFilePath);
        }
    ...
    if (result != 0) {
        fprintf(stderr, u8"Error creating Windows DLL library. Failed command: %s\n", cmd);
    }
...
static UPKGOptions *pkg_checkFlag(UPKGOptions *o) {
    ...
    const char MAP_FILE_EXT[] = u8".map";
    FileStream *f = NULL;
    char mapFile[SMALL_BUFFER_MAX_SIZE] = u8"";
    int32_t start = -1;
    uint32_t count = 0;
    const char rm_cmd[] = u8"rm -f all ;";
    ...
    if (uprv_strstr(flag, rm_cmd) != NULL) {
        ...
        if (tmpGenlibFlagBuffer == NULL) {
            ...
            fprintf(stderr,u8"Unable to allocate buffer of size: %d.\n", length);
            return NULL;
        }
    ...
    if (start >= 0) {
        ...
        f = T_FileStream_open(mapFile, u8"w");
        if (f == NULL) {
            fprintf(stderr,u8"Unable to create map file: %s.\n", mapFile);
            return NULL;
        } else {
            sprintf(tmpbuffer, u8"%s%s ", o->entryName, UDATA_CMN_INTERMEDIATE_SUFFIX);
...
static void loadLists(UPKGOptions *o, UErrorCode *status)
{
    ...
    for(l = o->fileListFiles; l; l = l->next) {
        if(o->verbose) {
            fprintf(stdout, u8"# pkgdata: Reading %s..\n", l->str);
        }
        /* TODO: stdin */
        in = T_FileStream_open(l->str, u8"r"); /* open files list */

        if(!in) {
            fprintf(stderr, u8"Error opening <%s>.\n", l->str);
        ...
        while(T_FileStream_readLine(in, line, sizeof(line))!=NULL) { /* for each line */
            ...
            if(uprv_strlen(line)>lineMax) {
                fprintf(stderr, u8"%s:%d - line too long (over %d chars)\n", l->str, (int)ln, (int)lineMax);
                exit(1);
            }
            ...
            while(linePtr && *linePtr) { /* process space-separated items */
                ...
                if(linePtr[0] == '\x22')
                {
                    lineNext = uprv_strchr(linePtr+1, '\x22');
                    if(lineNext == NULL) {
                        fprintf(stderr, u8"%s:%d - missing trailing double quote (\")\n",
                            l->str, (int)ln);
                        exit(1);
                    } else {
                        lineNext++;
                        if(*lineNext) {
                            if(*lineNext != '\x20') {
                                fprintf(stderr, u8"%s:%d - malformed quoted line at position %d, expected ' ' got '%c'\n",
                                    l->str, (int)ln, (int)(lineNext-line), (*lineNext)?*lineNext:'\x30');
                                exit(1);
                            }
                            *lineNext = 0;
                            lineNext++;
                    ...
                    lineNext = uprv_strchr(linePtr, '\x20');
                    if(lineNext) {
                ...
                if(uprv_pathIsAbsolute(s) || s[0] == '\x2e') {
                    fprintf(stderr, u8"pkgdata: Error: absolute path encountered. Old style paths are not supported. Use relative paths such as 'fur.res' or 'translit%cfur.res'.\n\tBad path: '%s'\n", U_FILE_SEP_CHAR, s);
                ...
                if((tmp = (char *)uprv_malloc(tmpLength)) == NULL) {
                    fprintf(stderr, u8"pkgdata: Error: Unable to allocate tmp buffer size: %d\n", tmpLength);
                    exit(U_MEMORY_ALLOCATION_ERROR);
                }
                uprv_strcpy(tmp, o->srcDir);
                uprv_strcat(tmp, o->srcDir[uprv_strlen(o->srcDir)-1] == U_FILE_SEP_CHAR ? u8"" : PKGDATA_FILE_SEP_STRING);
                uprv_strcat(tmp, s);
...
/* Try calling icu-config directly to get the option file. */
 static int32_t pkg_getOptionsFromICUConfig(UBool verbose, UOption *option) {
    ...
    size_t n;
    static char buf[512] = u8"";
    icu::CharString cmdBuf;
    UErrorCode status = U_ZERO_ERROR;
    const char cmd[] = u8"icu-config --incpkgdatafile";
    char dirBuf[1024] = u8"";
    ...
    if(U_SUCCESS(status)) {
      ...
      if(verbose) {
        fprintf(stdout, u8"# Calling icu-config: %s\n", cmdBuf.data());
      }
      p.adoptInstead(popen(cmdBuf.data(), u8"r"));
    }
    ...
    if(p.isNull() || (n = fread(buf, 1, UPRV_LENGTHOF(buf)-1, p.getAlias())) <= 0) {
        ...
        p.adoptInstead(popen(cmd, u8"r"));
        if(p.isNull() || (n = fread(buf, 1, UPRV_LENGTHOF(buf)-1, p.getAlias())) <= 0) {
            fprintf(stderr, u8"%s: icu-config: No icu-config found. (fix PATH or use -O option)\n", progname);
            return -1;
        }
    }

    for (int32_t length = strlen(buf) - 1; length >= 0; length--) {
        if (buf[length] == '\xa' || buf[length] == '\x20') {
            buf[length] = 0;
        } else {
            break;
        }
    ...
    if(buf[0] == 0)
    {
        fprintf(stderr, u8"%s: icu-config: invalid response from icu-config (fix PATH or use -O option)\n", progname);
        return -1;
    }

    if(verbose) {
      fprintf(stdout, u8"# icu-config said: %s\n", buf);
    }
...
static void pkg_createOptMatchArch(char *optMatchArch) {
#if !defined(WINDOWS_WITH_MSVC) || defined(USING_CYGWIN)
    const char* code = u8"void oma(){}";
    const char* source = u8"oma.c";
    const char* obj = u8"oma.obj";
    FileStream* stream = NULL;

    stream = T_FileStream_open(source,u8"w");
    if (stream != NULL) {
        T_FileStream_writeLine(stream, code);
        T_FileStream_close(stream);

        char cmd[LARGE_BUFFER_MAX_SIZE];
        sprintf(cmd, u8"%s %s -o %s",
            pkgDataFlags[COMPILER],
            source,
            obj);

        if (runCommand(cmd) == 0){
            sprintf(optMatchArch, u8"%s", obj);
        }
        else {
            fprintf(stderr, u8"Failed to compile %s\n", source);
        }
        if(!T_FileStream_remove(source)){
            fprintf(stderr, u8"T_FileStream_remove failed to delete %s\n", source);
        }
    }
    else {
        fprintf(stderr, u8"T_FileStream_open failed to open %s for writing\n", source);
    }
#endif
}
static void pkg_destroyOptMatchArch(char *optMatchArch) {
