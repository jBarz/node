    ...
    while(l != NULL)
    {
        if(l->str)
        {
            ...
            if(quote < 0) { /* remove quotes */
                if(buffer[uprv_strlen(buffer)-1] == '\x22') {
                    buffer[uprv_strlen(buffer)-1] = '\x0';
                }
                if(buffer[0] == '\x22') {
                ...
                if(l->str[0] != '\x22') {
                    uprv_strcpy(buffer, u8"\"");
                    uprv_strncat(buffer, l->str,1020);
                }
                if(l->str[uprv_strlen(l->str)-1] != '\x22') {
...
const char *pkg_writeCharList(FileStream *s, CharList *l, const char *delim, int32_t quote)
{
    ...
    while(l != NULL)
    {
        if(l->str)
        {
            ...
            {
                fprintf(stderr, u8"%s:%d: Internal error, line too long (greater than 1023 chars)\n",
                        __FILE__, __LINE__);
                exit(0);
            }
            if(quote < 0) { /* remove quotes */
...
CharList *pkg_appendUniqueDirToList(CharList *l, CharList** end, const char *strAlias) {
    ...
    if((rPtr-strAlias) >= UPRV_LENGTHOF(aBuf)) {
        fprintf(stderr, u8"## ERR: Path too long [%d chars]: %s\n", (int)sizeof(aBuf), strAlias);
        return l;
    }
