...
static const char copyRight[] =
    ...
    u8" *******************************************************************************\n"
    u8" *\n"
    u8" *   Copyright (C) International Business Machines\n"
...
static const char warningMsg[] =
    ...
    u8"######################################################################\n"
    u8"\n"
    u8"   WARNING: This file is generated by genrb Version " GENRB_VERSION u8".\n"
    u8"            If you edit this file, please make sure that, the source\n"
    u8"            of this file (XXXX.txt in LocaleElements_XXXX.java)\n"
    u8"            is also edited.\n"
    u8"######################################################################\n"
    u8" *********************************************************************\n"
    u8" */\n\n";
static const char* openBrace=u8"{\n";
static const char* closeClass=u8"    };\n"
                              u8"}\n";

static const char* javaClass =  u8"import java.util.ListResourceBundle;\n\n"
                                u8"public class ";

static const char* javaClass1=  u8" extends ListResourceBundle {\n\n"
                                u8"    /**\n"
                                u8"     * Overrides ListResourceBundle \n"
                                u8"     */\n"
                                u8"    public final Object[][] getContents() { \n"
                                u8"          return  contents;\n"
                                u8"    }\n\n"
                                u8"    private static Object[][] contents = {\n";
...
uCharsToChars(char *target, int32_t targetLen, const UChar *source, int32_t sourceLen, UErrorCode *status) {
    ...
    while(i<sourceLen){
        if (source[i] == '\xa') {
            if (j + 2 < targetLen) {
                uprv_strcat(target, u8"\\n");
            }
            j += 2;
        }else if(source[i]==0x0D){
            if(j+2<targetLen){
                uprv_strcat(target,u8"\\f");
            }
            j+=2;
        }else if(source[i] == '\x22'){
            if(source[i-1]=='\x27'){
                if(j+2<targetLen){
                    uprv_strcat(target,"\\");
            ...
            if(*enc =='\x0' || source[i]==0x0000){
                uprv_strcpy(str,u8"\\u");
...
str_write_java(const UChar *src, int32_t srcLen, UBool printEndLine, UErrorCode *status) {
    ...
    if(bufLen+(tabCount*4) > columnCount  ){
        ...
        while(len < bufLen){
            ...
            current = buf +len;
            if (add < (bufLen-len)) {
                uint32_t idx = strrch(current,add,'\x5c');
                if (idx > add) {
                    idx = add;
                } else {
                    int32_t num =idx-1;
                    ...
                    seqLen = (current[idx+1]=='\x75') ? 6 : 2;
                    if ((add-idx) < seqLen) {
                        add = idx + seqLen;
                    }
                }
            }
            T_FileStream_write(out,u8"\"",1);
            if(len+add<bufLen){
                T_FileStream_write(out,current,add);
                T_FileStream_write(out,u8"\" +\n",4);
                write_tabs(out);
            }else{
...
array_write_java(const ArrayResource *res, UErrorCode *status) {
    ...
    if (res->fCount > 0) {
        ...
        current = res->fFirst;
        if(allStrings==FALSE){
            const char* object = u8"new Object[]{\n";
            write_tabs(out);
...
intvector_write_java(const IntVectorResource *res, UErrorCode * /*status*/) {
    uint32_t i = 0;
    const char* intArr = u8"new int[] {\n";
    /* const char* intC   = "new Integer(";   */
    const char* stringArr = u8"new String[]{\n";
    ...
    if(resname != NULL && uprv_strcmp(resname,u8"DateTimeElements")==0){
...
int_write_java(const IntResource *res, UErrorCode * /*status*/) {
    const char* intC   =  u8"new Integer(";
    char buf[100];
    int len =0;
    buf[0]=0;
...
res_write_java(struct SResource *res,UErrorCode *status) {
    ...
    if (res != NULL) {
        ...
        case URES_STRING:
             ...
             return;
        case URES_ALIAS:
             printf(u8"Encountered unsupported resource type %d of alias\n", res->fType);
...
bundle_write_java(struct SRBRoot *bundle, const char *outputDir,const char* outputEnc,
    ...
    bName = (bundleName==NULL) ? u8"LocaleElements" : bundleName;
    pName = (packageName==NULL)? u8"com.ibm.icu.impl.data" : packageName;

    uprv_strcpy(className, bName);
    srBundle = bundle;
    if(uprv_strcmp(srBundle->fLocale,u8"root")!=0){
        uprv_strcat(className,u8"_");
    ...
    out= T_FileStream_open(fileName,u8"w");
    ...
    T_FileStream_write(out,u8"package ",(int32_t)uprv_strlen(u8"package "));
