...
static const char *kConverter = u8"ibm-1047";

int main(int argc, const char *argv[]) {
  printf(u8"// %s\n", U_COPYRIGHT_STRING);
  printf(u8"// generated by tblgen. You weren't going to edit it by hand, were you?\n");
  printf(u8"\n");
  ...
  if(U_FAILURE(status)) {
    fprintf(stderr, u8"Failed to open %s: %s\n", kConverter, u_errorName(status));
    return 1;
  }

  printf(u8"static const char cp1047_8859_1[256] = { \n");
  for(int i=0x00; i<0x100; i++) {
    ...
    if(U_FAILURE(status)) {
      fprintf(stderr, u8"Conversion failure at #%X: %s\n", i, u_errorName(status));
      return 2;
    }
    printf(u8" (char)0x%02X, /* %02X */\n", u[0], i);
  }
  printf(u8"};\n\n");

  //
  //  UnicodeSet oldIllegal("[:print:]", status); // [a-zA-Z0-9_}{#)(><%:;.?*+-/^&|~!=,\\u005b\\u005d\\u005c]", status);
  UnicodeSet oldIllegal(u8"[0-9 a-z A-Z "
                        u8"_ \\{ \\} \\[ \\] # \\( \\) < > % \\: ; . "
                        u8"? * + \\- / \\^ \\& | ~ ! = , \\ \" ' ]", status);
...
Some discussion at http://unicode.org/mail-arch/unicode-ml/y2003-m10/0471.html
  ...
  printf(u8"static const bool oldIllegal[256] = { \n");
  for(UChar i=0x00; i<0x100;i++) {
    printf(u8" %s, /* U+%04X */\n",
           (oldIllegal.contains(i))?u8" true":u8"false",
           i);
  }
  printf(u8"};\n\n");
