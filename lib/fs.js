...
const kMaxStringLength = 128 * 1024 * 1024;
...
function readFileAfterRead(err, bytesRead) {
  ...
  if (isZos) {
    ...
    var bytesWritten = 0;
    while (bytesLeft > 0) {
      var w = context.buffer.write(context.buffer.toString('ebcdic',
                                                           bytesWritten,
                                                           bytesWritten + (bytesLeft > kMaxStringLength ? kMaxStringLength : bytesLeft)),
                                   bytesWritten, 
                                   bytesLeft > kMaxStringLength ? kMaxStringLength : bytesLeft,
                                   'utf8');
      bytesLeft -= w;
      bytesWritten += w;
   }
...
fs.read = function(fd, buffer, offset, length, position, callback) {
  ...
  if (isZos) {
    ...
    while (bytesLeft > 0) {
      var w = buffer.write(buffer.toString('ebcdic',
                                           bytesWritten,
                                           bytesWritten + (bytesLeft > kMaxStringLength ? kMaxStringLength: bytesLeft)),
                           bytesWritten, 
                           bytesLeft > kMaxStringLength ? kMaxStringLength : bytesLeft,
...
fs.readSync = function(fd, buffer, offset, length, position) {
  ...
  if (isZos) {
    ...
    while (bytesLeft > 0) {
      var w = buffer.write(buffer.toString('ebcdic',
                                           offset+bytesWritten,
                                           offset + bytesWritten +(bytesLeft > kMaxStringLength ? kMaxStringLength: bytesLeft)),
                           offset+bytesWritten, 
                           bytesLeft > kMaxStringLength ? kMaxStringLength : bytesLeft,
                           'utf8');
      bytesLeft -= w;
