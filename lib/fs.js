...
function readFileAfterClose(err) {
  ...
  if (context.pos > 0 && process.platform === 'os390')
    buffer.write(buffer.toString('ebcdic', 0, context.pos), 0, context.pos, 'latin1');
...
fs.read = function(fd, buffer, offset, length, position, callback) {
  ...
  function wrapper(err, bytesRead) {
    ...
    if (bytesRead > 0 && process.platform === 'os390')
      buffer.write(buffer.toString('ebcdic', offset, offset+bytesRead), offset, bytesRead, 'latin1');
...
fs.readSync = function(fd, buffer, offset, length, position) {
  ...
  if (r > 0 && process.platform === 'os390') 
    buffer.write(buffer.toString('ebcdic', offset, offset+r), offset, r, 'latin1');
  if (!legacy) {
    return r;
  }
